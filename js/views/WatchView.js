...
export function renderWatch(container, title, onBack, onPrev, onNext, onFullscreen, onSwitchProxy, similarAnimes, onCardClick) {
  container.innerHTML = `
    <div id="watchView" class="hidden max-w-7xl mx-auto px-6 py-8">
      <div class="flex items-center justify-between mb-6">
        <button onclick="${onBack}" class="flex items-center gap-3 text-blue-400 hover:text-white font-medium">← Back to Details</button>
      ...
      <div class="flex flex-wrap justify-center gap-4">
        ...
        <button onclick="${onFullscreen}" class="flex items-center gap-3 bg-zinc-800 hover:bg-zinc-700 px-6 py-3 rounded-2xl font-medium transition text-sm sm:text-base"><i class="fa-solid fa-expand"></i> Full Screen</button>
        <button onclick="${onSwitchProxy}" class="flex items-center gap-3 bg-zinc-800 hover:bg-zinc-700 px-6 py-3 rounded-2xl font-medium transition text-sm sm:text-base"><i class="fa-solid fa-rotate"></i> Switch Streaming Node</button>
